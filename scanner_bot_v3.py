...
# ── Zona horaria ET (bolsa de Nueva York) ─────────────────────
ET = timezone(timedelta(hours=-4))   # EDT marzo-noviembre
def hora_et():
...
CONFIG = {
    "capital_usd":        33140,
    "riesgo_fijo_usd":    100,     # riesgo FIJO en dolares por trade
    "stop_loss_pct":      6.0,
    "price_min":          10.0,
    "price_max":          150.0,
    "rsi_min":            45,
    "rsi_max":            70,      # bajado de 72 a 70 — RSI 70 ya es limite
...
def obtener_datos(ticker):
    try:
        ...
        return {
            ...
            "ema10":float(ema(c,10).iloc[-1]),"ema20":float(ema(c,20).iloc[-1]),
            "ema50":float(ema(c,50).iloc[-1]),"ema200":float(ema(c,200).iloc[-1]),
            "rsi":calc_rsi(c),
...
def analizar(d):
    ...
    return {"fan":fan,"estado":estado,"c1":c1,"c2":c2,"c3":c3,"c4":c4,
...
def posicion(precio):
    """
    Calcula la posicion con RIESGO FIJO en dolares (no porcentaje).
    Riesgo fijo = CONFIG["riesgo_fijo_usd"] = 100 USD
    """
    riesgo   = CONFIG["riesgo_fijo_usd"]      # 100 USD exactos
    sp       = CONFIG["stop_loss_pct"] / 100   # 0.06
    stop     = precio * (1 - sp)               # precio de stop
    rx       = precio - stop                   # riesgo por accion
    acc      = max(1, int(riesgo / rx))        # acciones para arriesgar exactamente $100
    tot      = acc * precio                    # capital total en la posicion
    t1       = precio * 1.12                   # target +12%
    t2       = precio * 1.22                   # target +22%
    rr       = (t1 - precio) / rx             # ratio R/R
    perd_real = acc * rx                       # perdida real (siempre ~100)
    return {
        "acc":acc,"tot":tot,"stop":stop,
        "perd":perd_real,"t1":t1,"t2":t2,"rr":rr
    }
...
def analizar_ia(d, a, pos):
    ...
    try:
        ...
        prompt=(
            ...
            f"Responde EXACTAMENTE en este formato (sin simbolo dolar):\n"
        ...
        msg=client.messages.create(
            ...
            system="Analizador Sistema Maestro v4. Sin simbolos especiales. Español.",
...
def build_msg(d, a, pos, ia):
    # Sin caracteres especiales raros — todo ASCII seguro para WhatsApp
    hora   = hora_et()
    pm_tag = " [PRE-MARKET]" if d.get("es_pm") else ""
    ef     = "OK 4/4" if a["fan"]==4 else f"PARCIAL {a['fan']}/4"
    emoji  = {"ENTRAR":"ENTRAR","ESPERAR":"ESPERAR","NO APLICA":"NO APLICA"}.get(ia["senal"], ia["senal"])
    rsi_v  = d["rsi"] if d["rsi"] else 0
    vol_ok = "OK" if a["vol_r"]>=1.5 else "bajo"
    rsi_ok = "OK" if a["rsi_ok"] else "fuera de rango"

    # Precios sin simbolo $ — usando USD para evitar corte en WhatsApp
    entrada = f"{d['precio']:.2f} USD"
    stop    = f"{pos['stop']:.2f} USD"
    t1      = f"{pos['t1']:.2f} USD"
    t2      = f"{pos['t2']:.2f} USD"
    ...
    return (
        ...
        f"*Indicadores*\n"
        f"MACD: {d['macd_e']}\n"
        f"RSI:  {rsi_v:.0f}  ({rsi_ok})\n"
        f"ADX:  {d['adx_e']} ({d['adx']:.0f})\n"
        f"Vol:  {a['vol_r']:.1f}x promedio ({vol_ok})\n\n"
        f"*Tu posicion  (riesgo fijo 100 USD)*\n"
        f"Precio entrada: {entrada}\n"
        f"Comprar: *{pos['acc']} acciones*  (capital {pos['tot']:.0f} USD)\n"
        f"Stop loss:  {stop}  (-6%)\n"
        f"Target 1:   {t1}  (+12%)\n"
        f"Target 2:   {t2}  (+22%)\n"
        f"R/R: {pos['rr']:.1f}x   Perdida maxima: {pos['perd']:.0f} USD\n\n"
        f"*IA {ia['prob']}%  -  {emoji}*\n"
        f"{ia['razon']}\n"
        f"Vigilar: {ia['alerta']}\n\n"
...
def main():
    ...
    ya=estado.get("alertados",[]); count=estado.get("count",0)
    ...
    for ticker in tickers:
        ...
        a=analizar(d)
        if a["fan"]<CONFIG["min_fan_to_alert"] or not a["en_rango"]: continue
    ...
    if nuevas==0 and hora_utc==13 and count==0:
        send_wa(
            f"SISTEMA LU\n{hora_et()}\n\n"
            f"Revise {len(CONFIG['watchlist_prioritaria'])} tickers prioritarios.\n"
